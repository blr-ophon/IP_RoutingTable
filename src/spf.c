...
//Takes Dijkstra's table filled and transforms into routing table of router id
void SPF_fillL3Table(struct Router *routers, struct table_entry *dist_table, int id){
    ...
    //Find the gateway for all the other routers in the topology
    for(int i = 0; i < ROUTER_TOTAL; i++){

        //visit previous node until a direct neighbor of id is found
        int tmp_i = i;
        while(dist_table[tmp_i].pv_node != id){
            tmp_i = dist_table[tmp_i].pv_node;
        }

        //insert address and gateway in routing table
